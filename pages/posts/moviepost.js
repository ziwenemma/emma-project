       ...
       <div className='p-10 grid grid-cols-2 gap-3 mx-auto'>
         {playlistLists.map(playlistLists =>(
          ...
          <div className='w-full'>
             <img src={playlistLists.image} alt=''width='300px' height='200px'/></div>
        ...
        <div class="pt-4 pb-2">
        <span class="inline-block bg-blue-500 rounded-full  py-1 text-sm font-semibold text-blue-700 mr-2 mb-2">{playlistLists.info.date}</span>
        <span class="inline-block bg-pruple-400 bg-opacity-60 rounded-full px-3 py-1 text-sm font-semibold text-gray-700 mr-2 mb-2">{playlistLists.info.category}</span>
      </div>
           </div>
         ))}
    </div>
